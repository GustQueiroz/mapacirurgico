...
export class Schedule implements DoCheck, OnDestroy, OnInit, OnChanges, AfterViewChecked {
  ...
  @Input() slotLabelFormat: string | null = 'H:mm';
  ...
  ngOnInit() {
    ...
    this.config = {
      ...
      { 
          let html='';
          if(event.statusAgendamentoStr == 'Confirmado')
            html = 'label label-success';
          else if(event.statusAgendamentoStr =='Cancelado')
            html = 'label label-danger';
          
          element.find('.fc-title').append("<br/><span class='"+html+"'>(" + event.statusAgendamentoStr +")</span>"); 
      },
